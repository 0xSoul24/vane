...
class RegionExtent {
    ...
    fun isInside(loc: Location): Boolean {
        if (loc.getWorld() != min()!!.world) {
            return false
        }

        val l = min()
        val h = max()
        return (loc.x >= l!!.x && loc.x < (h!!.x + 1) && loc.y >= l.y && loc.y < (h.y + 1) && loc.z >= l.z && loc.z < (h.z + 1)
                )
    }

    fun isInside(block: Block): Boolean {
        if (block.world != min()!!.world) {
            return false
        }

        val l = min()
        val h = max()
        return (block.x >= l!!.x && block.x <= h!!.x && block.y >= l.y && block.y <= h.y && block.z >= l.z && block.z <= h.z
                )
    }

    fun intersectsExtent(other: RegionExtent): Boolean {
        if (min()!!.world != other.min()!!.world) {
            return false
        }
