...
public class MenuFactory {
    ...
    public static Menu anvil_string_input(
        ...
        anvil.add(new MenuItemClickListener(2, (p, menu, item) -> on_click.apply(p, menu, name_of(item))));
        return anvil;
    }
        ...
        for (int i = 0; i < columns; ++i) {
            if (i == confirm_index) {
                confirmation_menu.add(
                        new MenuItem(i, item_confirm, (player, menu, self) -> {
                            menu.close(player);
                ...
                confirmation_menu.add(
                        new MenuItem(i, item_cancel, (player, menu, self) -> {
                            menu.close(player);
    ...
    public static Menu item_selector(
        ...
        // Accept item
        item_selector_menu.add(
                new MenuItem(2, menu_manager.item_selector_accept.item(), (p, menu, self) -> {
        ...
        // Cancel item
        item_selector_menu.add(
                new MenuItem(6, menu_manager.item_selector_cancel.item(), (p, menu, self) -> {
                    menu.close(p);
                    on_cancel.apply(player);
                    return ClickResult.SUCCESS;
                })
    ...
    public static Menu head_selector(
        ...
        final var menu_manager = context.get_module().core.menu_manager;
        final var all_heads = HeadMaterialLibrary.all()
                .stream()
