...
public interface CustomItem {
     ...
     * subclass to always return false in {@link #enabled()}. This will cause existing items to
     * behave as their base items. To completely remove a custom item from encountered inventories,
     * you can queue it for removal using {@link
     * CustomItemRegistry#removePermanently(NamespacedKey)}. The key is the primary identifier of an
    ...
    /**
     * Returns a version number of this item. Increasing this value will cause {@link
     * #convertExistingStack(ItemStack)} to be called on any encountered itemstack of this custom
     * item. Useful to force an update of existing items when you, for example, change the {@link
     * #updateItemStack(ItemStack)} function to add custom properties to your custom item.
     */
    public int version();
     ...
     *
     * <p>To ensure that a breakage of the plugin never creates value for players, use a material
     ...
     *
     * <p>By default, no attempts will be made to remove the vanilla behavior of the base items,
     * except for inhibiting use in crafting recipes that require the base material. See {@link
     * #inhibitedBehaviors()} for more information. If you require other behaviors to be inhibited,
     ...
     * {@link CustomModelDataRegistry#reserveRange(NamespacedKey, int, int)} when your plugin
     ...
     * {@link #updateItemStack(ItemStack)}. By using translatable components, there's no need to
     ...
     *
     * <p>If this value is changed while item stacks of this custom item already exist with a
     ...
     *
     * <p>By default, any _vanilla_ crafting recipe (i.e. "minecraft" is the associated key's
     ...
     *
     * <p>So a custom item based on paper as the base material cannot be used to craft, for example,
     ...
     *
     * <p>If you require a hoe that doesn't till blocks or a carrot/fungus on a stick that doesn't
     * attract certain entities, you can override {@link #inhibitedBehaviors()} to specify what
     * should be prohibited. The user must handle anything else.
     */
    public default EnumSet<InhibitBehavior> inhibitedBehaviors() {
     ...
     */
    public default ItemStack updateItemStack(@NotNull final ItemStack itemStack) {
        return itemStack;
    }
