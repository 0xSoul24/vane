...
import static org.oddlama.vane.util.BlockUtil.textureFromSkull;
...
public class HeadLibrary extends Listener<Core> {
    ...
    public boolean configPlayerHeadDrops;

    public HeadLibrary(Context<Core> context) {
        super(context);
        // Load a head material library
        getModule().log.info("Loading head library...");
        try {
            String json = null;
            try (var input = getModule().getResource("head_library.json")) {
                if (input != null) {
            ...
            getModule().log.log(Level.SEVERE, "Error while loading head_library.json! Shutting down.", e);
    ...
    public void onBlockBreak(final BlockBreakEvent event) {
        ...
        final var texture = textureFromSkull(skull);
        if (texture == null) {
            return;
        }

        final var headMaterial = HeadMaterialLibrary.fromTexture(texture);
        ...
        BlockUtil.dropNaturally(block, headMaterial.item());
    }
}