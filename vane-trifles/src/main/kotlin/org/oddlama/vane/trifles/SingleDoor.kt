 ...
 */
class SingleDoor private constructor(// Must be the lower block
    ...
    private fun findOtherDoor(hacked: Boolean): SingleDoor? {
        val otherDoorDirection = this.otherDoorDirection(lower!!, hacked)
    ...
    var isOpen: Boolean
        get() = lower!!.isOpen
        set(open) {
            val data: Door = asDoorState(lowerBlock)!!
            data.isOpen = open
    ...
    companion object {
        ...
        private fun getLower(originBlock: Block): Block {
            val half: Bisected.Half = asDoorState(originBlock)!!.half
            return when (half) {
        ...
        private fun otherVerticalHalf(block: Block): Block {
            val door: Door = asDoorState(block)!!
        ...
        private fun doorVerticalHalvesMatch(originBlock: Block, otherBlock: Block): Boolean {
            val originState: Door = asDoorState(originBlock)!!
